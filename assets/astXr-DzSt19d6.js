import{W as Ve,E as Ye,S as Qe,C as pe,A as Ke,V as l,H as en,T as Le,a as G,Q as q,R as Ne,M as V,b as y,c as nn,d as W,D as Ee,p as xe,e as tn}from"./package-BjbQfw9P.js";const Se=`import "@babylonjs/loaders/glTF";
...
const defaultOpenMarchUrl = "/examples/openmarch-large-example.om";
...
interface PreparedDrill {
  ...
  setNoun: "Page" | "Sheet";
  title: string;
  tempoSections?: Array<{ tempo: number; numberOfBeats: number }>;
  playbackRate: number;
}
...
function prepareCompiledDrill(file: CompiledDrill): void {
  ...
  const tempoSections = file.tempoSections?.flatMap((section, index) => {
    const tempo = finiteNumber(section.tempo, \`tempo section \${index + 1} tempo\`);
    const numberOfBeats = finiteNumber(section.numberOfBeats, \`tempo section \${index + 1} beats\`);
    if (tempo < 0 || numberOfBeats < 0) throw new Error(\`Compiled drill tempo section \${index + 1} cannot have negative tempo or beats\`);
    return tempo === 0 || numberOfBeats === 0 ? [] : [{ tempo, numberOfBeats }];
  });
  const playbackRate = file.playbackRate === undefined ? 1 : finiteNumber(file.playbackRate, "playback rate");
  if (playbackRate <= 0) throw new Error("Compiled drill playback rate must be positive");
  let startBeat = 0;
  ...
  preparedDrill = { labels, sets, setNoun: file.setNoun, title: file.title, tempoSections: tempoSections?.length ? tempoSections : undefined, playbackRate };
...
function sampleCompiledMovement(commands: CompiledDrillMovement[], beat: number, sample: CompiledMovementSample): void {
  ...
  if (command.type === "even") {
    ...
    sample.gait = progress < 1 ? "high-step" : "stand";
...
function tempoMapBeat(seconds: number, tempoSections: PreparedDrill["tempoSections"]): number {
  if (!tempoSections) return loadedMidi ? Math.max(0, loadedMidi.header.secondsToTicks(seconds) / loadedMidi.header.ppq) : 0;
  let remainingSeconds = Math.max(0, seconds);
  let beat = 0;
  for (const section of tempoSections) {
    const sectionSeconds = section.numberOfBeats * 60 / section.tempo;
    if (remainingSeconds < sectionSeconds) return beat + remainingSeconds * section.tempo / 60;
    beat += section.numberOfBeats;
    remainingSeconds -= sectionSeconds;
  }
  return beat;
}

function compiledDrillState(seconds: number, drill: PreparedDrill): DrillPlaybackState {
  const beat = tempoMapBeat(seconds * drill.playbackRate, drill.tempoSections);
  ...
  if (setIndex !== compiledFrameSetIndex) {
    ...
    canvas.dataset.drillSetIndex = String(setIndex);
  }
...
function currentDrillState(ticks: number, seconds: number): DrillPlaybackState {
  return preparedDrill ? compiledDrillState(seconds, preparedDrill) : nativeDrillState(ticks);
}
...
function stationaryLegLifts(phase: number): [number, number] {
  const lift = Math.sin(phase);
  return [Math.max(0, lift), Math.max(0, -lift)];
}
...
function animatePerformerRig(
  ...
  const kneeLift = gait === "high-step" || (!motion.moving && gait === "mark-time");
  const stepping = playing && (motion.moving || kneeLift);
  ...
  if (kneeLift && playing) {
    const [leftLift, rightLift] = stationaryLegLifts(phase);
    rig.leftLeg.rotation.x = leftLift * profile.legSwing;
    rig.rightLeg.rotation.x = rightLift * profile.legSwing;
    rig.leftLeg.rotation.z = 0;
    rig.rightLeg.rotation.z = 0;
  } else if (motion.moving && gait === "crab-step") {
...
function animateSimplifiedPerformerRig(
  ...
  if (kneeLift && playing) {
    const [leftLift, rightLift] = stationaryLegLifts(phase);
    rig.leftLeg.rotation.set(leftLift * 0.52, 0, 0);
    rig.rightLeg.rotation.set(rightLift * 0.52, 0, 0);
  } else {
...
scene.onBeforeRenderObservable.add(() => {
  ...
  const drill = currentDrillState(ticks, seconds);
  drawScoreboard(drill);
  const stepTicks = loadedMidi ? loadedMidi.header.ppq * 2 : 1;
  const stepPhase = preparedDrill
    ? tempoMapBeat(seconds * preparedDrill.playbackRate, preparedDrill.tempoSections) * Math.PI
    : ticks / stepTicks * Math.PI * 2;
...
void fetch("/music/the-stars-and-stripes-forever.mid")
  ...
  .then(() => fetch(performanceTestPerformerCount === null ? defaultOpenMarchUrl : queenFinalShwUrl))
  .then(response => response.arrayBuffer())
  .then(async data => {
    if (defaultDrillGeneration !== drillLoadGeneration) return;
    if (performanceTestPerformerCount === null) await prepareOpenMarch(data);
    else prepareCalChartShw(data, "queen-final.shw");
  })
  .catch(error => setStatus(error instanceof Error ? error.message : "Unable to load the default music"));
void initializeXr();
`,an={kind:"SourceFile",label:"chart-xr.ts",start:0,end:95273,uses:[3040,2505,5137,5927,3926,6258,6580,5049,6761,7084,7319,9014,13027,13797,26357,34371,34118,34868,35467,75052,74759,74799,75191,74837,75107,74909,4162,76867,4249,4336,4416,81434,75795,4498,79151,4578,79508,4658,35433,35572,35544,4080,73977,72827,92954,92516,92612,43320,4882,1203,1095,77659],usedBy:[2505,2664,3040,3415,5049,5137,5873,5927,6761,7084,7319,7481,7768,7838,7921,8004,8079,8149,8224,8313,8391,8459,8531,8608,8683,8754,8846,8930,9014,9166,9222,9652,10085,11870,12163,13797,14996,24834,26357,26868,27090,27481,34371,34868,35433,35467,35572,35683,36356,38927,39423,40733,42721,43320,44521,45338,48575,49476,49789,49972,51855,52713,53406,56410,59659,60762,61145,61902,62587,64323,64411,64529,64616,64805,66752,67065,70883,74512,74759,74799,74837,75881,77659,79151,79868,82141,82747,83780,84593,89936,95012],children:[{kind:"VariableStatement",label:"variable queenFinalShwUrl",start:1095,end:1202,uses:[],usedBy:[0],children:[]},{kind:"VariableStatement",label:"variable defaultOpenMarchUrl",start:1203,end:1270,uses:[],usedBy:[0],children:[]},{kind:"InterfaceDeclaration",label:"interface PrecalculatedFootstep",start:2505,end:2662,uses:[0],usedBy:[0,62587,64805],children:[]},{kind:"InterfaceDeclaration",label:"interface PerformerRig",start:2664,end:3038,uses:[0],usedBy:[14996,84593,89936],children:[]},{kind:"InterfaceDeclaration",label:"interface DrillCueFile",start:3040,end:3346,uses:[0],usedBy:[0,40733,44144,49789,64805],children:[]},{kind:"InterfaceDeclaration",label:"interface PreparedDrill",start:3415,end:3627,uses:[0],usedBy:[35720,70293,70883,93094],children:[]},{kind:"VariableStatement",label:"variable canvas",start:3926,end:4003,uses:[],usedBy:[5049,0,35830,70883],children:[]},{kind:"VariableStatement",label:"variable statusElement",start:4004,end:4079,uses:[],usedBy:[75795],children:[]},{kind:"VariableStatement",label:"variable buildInfoElement",start:4080,end:4161,uses:[],usedBy:[0],children:[]},{kind:"VariableStatement",label:"variable enterArButton",start:4162,end:4248,uses:[],usedBy:[75881,0],children:[]},{kind:"VariableStatement",label:"variable enterVrButton",start:4249,end:4335,uses:[],usedBy:[75881,0],children:[]},{kind:"VariableStatement",label:"variable midiFileInput",start:4336,end:4415,uses:[],usedBy:[27090,0,81168],children:[]},{kind:"VariableStatement",label:"variable drillFileInput",start:4416,end:4497,uses:[],usedBy:[0,81434],children:[]},{kind:"VariableStatement",label:"variable playButton",start:4498,end:4577,uses:[],usedBy:[43320,45338,79151,79508,0],children:[]},{kind:"VariableStatement",label:"variable stopButton",start:4578,end:4657,uses:[],usedBy:[43320,0],children:[]},{kind:"VariableStatement",label:"variable musicPositionInput",start:4658,end:4747,uses:[],usedBy:[43320,45338,0,94299],children:[]},{kind:"VariableStatement",label:"variable requestedPerformanceTestCount",start:4748,end:4881,uses:[],usedBy:[4882],children:[]},{kind:"VariableStatement",label:"variable performanceTestPerformerCount",start:4882,end:5047,uses:[4748],usedBy:[48575,0,95012],children:[]},{kind:"VariableStatement",label:"variable engine",start:5049,end:5136,uses:[0,3926],usedBy:[5137,0,84593,92612],children:[]},{kind:"VariableStatement",label:"variable scene",start:5137,end:5169,uses:[0,5049],usedBy:[0,5873,5927,6535,6761,7084,7319,7481,10085,11870,12163,13797,14996,26357,27090,77659,92665],children:[]},{kind:"VariableStatement",label:"variable fieldLength",start:5263,end:5286,uses:[],usedBy:[5337,5423,5468,10085,13027,13797,27090],children:[]},{kind:"VariableStatement",label:"variable fieldWidth",start:5287,end:5312,uses:[],usedBy:[10085,13027,26357,27090],children:[]},{kind:"VariableStatement",label:"variable endZoneDepth",start:5313,end:5336,uses:[],usedBy:[5337,10085,27090],children:[]},{kind:"VariableStatement",label:"variable playingFieldLength",start:5337,end:5395,uses:[5263,5313],usedBy:[10085,13027],children:[]},{kind:"VariableStatement",label:"variable tabletopScale",start:5396,end:5422,uses:[],usedBy:[34769,34819,73977],children:[]},{kind:"VariableStatement",label:"variable lifeSizeScale",start:5423,end:5467,uses:[5263],usedBy:[5796,73977],children:[]},{kind:"VariableStatement",label:"variable yardsPerSceneUnit",start:5468,end:5512,uses:[5263],usedBy:[5513],children:[]},{kind:"VariableStatement",label:"variable eightToFiveStep",start:5513,end:5563,uses:[5468],usedBy:[5564,66752,67065,84593],children:[]},{kind:"VariableStatement",label:"variable fourStepInterval",start:5564,end:5609,uses:[5513],usedBy:[5610,5654],children:[]},{kind:"VariableStatement",label:"variable formationInterval",start:5610,end:5653,uses:[5564],usedBy:[24834,53406],children:[]},{kind:"VariableStatement",label:"variable rankInterval",start:5654,end:5692,uses:[5564],usedBy:[24834,53406],children:[]},{kind:"VariableStatement",label:"variable performerModelHeight",start:5693,end:5728,uses:[],usedBy:[5796],children:[]},{kind:"VariableStatement",label:"variable shoeSoleClearance",start:5729,end:5762,uses:[],usedBy:[84593,89936],children:[]},{kind:"VariableStatement",label:"variable fieldPaintHeight",start:5763,end:5795,uses:[],usedBy:[10085,11870,12163,84593,89936],children:[]},{kind:"VariableStatement",label:"variable physicalPerformerScale",start:5796,end:5872,uses:[5693,5423],usedBy:[35830,73977],children:[]},{kind:"VariableStatement",label:"variable fieldRoot",start:5873,end:5925,uses:[0,5137],usedBy:[10085,11870,12163,13797,14996,26357,27090,34868,73977,76275],children:[]},{kind:"VariableStatement",label:"variable camera",start:5927,end:6019,uses:[0,5137],usedBy:[0,6535],children:[]},{kind:"VariableStatement",label:"variable flyKeys",start:6258,end:6292,uses:[],usedBy:[0,6580],children:[]},{kind:"IfStatement",label:"IfStatement",start:6535,end:6577,uses:[5137,5927],usedBy:[],children:[{kind:"ReturnStatement",label:"ReturnStatement",start:6570,end:6577,uses:[],usedBy:[],children:[]}]},{kind:"VariableStatement",label:"variable verticalDirection",start:6580,end:6665,uses:[6258],usedBy:[0],children:[]},{kind:"VariableStatement",label:"variable skyMaterial",start:6761,end:6828,uses:[0,5137],usedBy:[0],children:[]},{kind:"VariableStatement",label:"variable skybox",start:7084,end:7193,uses:[0,5137],usedBy:[0,73977,92665],children:[]},{kind:"VariableStatement",label:"variable light",start:7319,end:7405,uses:[0,5137],usedBy:[0],children:[]},{kind:"FunctionDeclaration",label:"function material",start:7481,end:7766,uses:[0,5137],usedBy:[7768,7838,7921,8004,8079,8149,8224,8313,8391,8459,8531,8608,8683,8754,8846,8930,9014],children:[]},{kind:"VariableStatement",label:"variable turfMaterial",start:7768,end:7837,uses:[7481,0],usedBy:[10085],children:[]},{kind:"VariableStatement",label:"variable turfStripeMaterial",start:7838,end:7920,uses:[7481,0],usedBy:[10085],children:[]},{kind:"VariableStatement",label:"variable lineMaterial",start:7921,end:8003,uses:[7481,0],usedBy:[11870],children:[]},{kind:"VariableStatement",label:"variable endZoneMaterial",start:8004,end:8078,uses:[7481,0],usedBy:[10085,14996],children:[]},{kind:"VariableStatement",label:"variable trackMaterial",start:8079,end:8148,uses:[7481,0],usedBy:[10085],children:[]},{kind:"VariableStatement",label:"variable uniformMaterial",start:8149,end:8223,uses:[7481,0],usedBy:[14996],children:[]},{kind:"VariableStatement",label:"variable uniformTrimMaterial",start:8224,end:8312,uses:[7481,0],usedBy:[14996],children:[]},{kind:"VariableStatement",label:"variable pantsMaterial",start:8313,end:8390,uses:[7481,0],usedBy:[14996],children:[]},{kind:"VariableStatement",label:"variable skinMaterial",start:8391,end:8458,uses:[7481,0],usedBy:[14996],children:[]},{kind:"VariableStatement",label:"variable shoeMaterial",start:8459,end:8530,uses:[7481,0],usedBy:[14996],children:[]},{kind:"VariableStatement",label:"variable brassMaterial",start:8531,end:8607,uses:[7481,0],usedBy:[14996],children:[]},{kind:"VariableStatement",label:"variable drumMaterial",start:8608,end:8682,uses:[7481,0],usedBy:[14996],children:[]},{kind:"VariableStatement",label:"variable standMaterial",start:8683,end:8753,uses:[7481,0],usedBy:[26357],children:[]},{kind:"VariableStatement",label:"variable scoreboardFrameMaterial",start:8754,end:8845,uses:[7481,0],usedBy:[27090],children:[]},{kind:"VariableStatement",label:"variable goalpostMaterial",start:8846,end:8929,uses:[7481,0],usedBy:[13797],children:[]},{kind:"VariableStatement",label:"variable tabletopMaterial",start:8930,end:9013,uses:[7481,0],usedBy:[10085],children:[]},{kind:"VariableStatement",label:"variable tabletopGrabMaterial",start:9014,end:9085,uses:[7481,0],usedBy:[0,10085],children:[]},{kind:"VariableStatement",label:"variable sharedPerformerMeshes",start:9166,end:9220,uses:[0],usedBy:[9222],children:[]},{kind:"FunctionDeclaration",label:"function createSharedPerformerMesh",start:9222,end:9650,uses:[0,9166],usedBy:[9652,14996],children:[]},{kind:"FunctionDeclaration",label:"function createMergedPerformerMesh",start:9652,end:10083,uses:[0,9222],usedBy:[14996],children:[]},{kind:"FunctionDeclaration",label:"function createField",start:10085,end:11868,uses:[0,5263,5287,5137,5873,8930,8079,7768,5337,5763,7838,5313,8004,9014],usedBy:[34118],children:[]},{kind:"FunctionDeclaration",label:"function addMarking",start:11870,end:12161,uses:[0,5137,5873,5763,7921],usedBy:[13027],children:[]},{kind:"FunctionDeclaration",label:"function addYardNumber",start:12163,end:13025,uses:[0,5137,5873,5763],usedBy:[13027],children:[]},{kind:"FunctionDeclaration",label:"function createFieldMarkings",start:13027,end:13795,uses:[11870,5287,5263,5337,12163],usedBy:[0],children:[]},{kind:"FunctionDeclaration",label:"function createGoalPosts",start:13797,end:14994,uses:[0,5137,5873,5263,8846],usedBy:[0],children:[]},{kind:"FunctionDeclaration",label:"function createPerformer",start:14996,end:24832,uses:[0,5137,5873,9222,8313,8149,8391,8608,8459,8531,9652,8224,8004,2664],usedBy:[24834,35830],children:[]},{kind:"FunctionDeclaration",label:"function createBand",start:24834,end:26355,uses:[0,5610,5654,14996],usedBy:[34244],children:[]},{kind:"FunctionDeclaration",label:"function createStands",start:26357,end:26784,uses:[0,5137,5873,5287,8683],usedBy:[0],children:[]},{kind:"FunctionDeclaration",label:"function configureScoreboardTexture",start:26868,end:27088,uses:[0],usedBy:[27090],children:[]},{kind:"FunctionDeclaration",label:"function createScoreboard",start:27090,end:34116,uses:[0,5263,5313,5287,5137,5873,8754,26868,4336,79645,79151,79508,29931,27481],usedBy:[27481,34325],children:[{kind:"FunctionDeclaration",label:"function positionScoreboard",start:27481,end:28421,uses:[0,27090],usedBy:[27090],children:[]},{kind:"FunctionDeclaration",label:"function drawControlLabel",start:29931,end:30411,uses:[],usedBy:[27090],children:[]}]},{kind:"VariableStatement",label:"variable { field, tabletopBase, tabletopGrabSurface }",start:34118,end:34185,uses:[10085],usedBy:[0,77659],children:[]},{kind:"VariableStatement",label:"variable performers",start:34244,end:34276,uses:[24834],usedBy:[34277,35830,73977,93636],children:[]},{kind:"VariableStatement",label:"variable nativePerformerCount",start:34277,end:34324,uses:[34244],usedBy:[40733,43320,64805],children:[]},{kind:"VariableStatement",label:"variable scoreboardTexture",start:34325,end:34370,uses:[27090],usedBy:[72827],children:[]},{kind:"VariableStatement",label:"variable tabletopDragBehavior",start:34371,end:34425,uses:[0],usedBy:[0,76275],children:[]},{kind:"VariableStatement",label:"variable tabletopDragAttached",start:34735,end:34768,uses:[],usedBy:[76275],children:[]},{kind:"VariableStatement",label:"variable minimumTabletopScale",start:34769,end:34818,uses:[5396],usedBy:[34868],children:[]},{kind:"VariableStatement",label:"variable maximumTabletopScale",start:34819,end:34866,uses:[5396],usedBy:[34868],children:[]},{kind:"FunctionDeclaration",label:"function lockTabletopLevel",start:34868,end:35303,uses:[5873,0,34819,34769],usedBy:[0,76275],children:[]},{kind:"VariableStatement",label:"variable transport",start:35433,end:35466,uses:[0],usedBy:[42721,45338,79151,79508,79645,0,92774,93263],children:[]},{kind:"VariableStatement",label:"variable synth",start:35467,end:35518,uses:[0],usedBy:[0,42721,45338,79508,79645],children:[]},{kind:"VariableStatement",label:"variable musicRepeatCount",start:35544,end:35571,uses:[],usedBy:[36356,42721,43320,79151,79645,0,92774,94299],children:[]},{kind:"VariableStatement",label:"variable loadedMidi",start:35572,end:35607,uses:[0],usedBy:[40733,43320,45338,64805,70293,79151,79645,82489,0,92774,92878,93030,94299],children:[]},{kind:"VariableStatement",label:"variable loadedMidiName",start:35608,end:35646,uses:[],usedBy:[43320],children:[]},{kind:"VariableStatement",label:"variable drillTitle",start:35647,end:35682,uses:[],usedBy:[40733,43320,44144,45338,49476,72827],children:[]},{kind:"VariableStatement",label:"variable drillCues",start:35683,end:35719,uses:[0],usedBy:[40733,43320,44144,45338,64805],children:[]},{kind:"VariableStatement",label:"variable preparedDrill",start:35720,end:35767,uses:[3415],usedBy:[40733,43320,45338,72648,93094],children:[]},{kind:"VariableStatement",label:"variable drillLoadGeneration",start:35768,end:35796,uses:[],usedBy:[81497,81623,81908,82141,94554,94948],children:[]},{kind:"VariableStatement",label:"variable lastScoreboardContent",start:35797,end:35828,uses:[],usedBy:[72827],children:[]},{kind:"FunctionDeclaration",label:"function setActivePerformerCount",start:35830,end:36354,uses:[34244,14996,5796,3926],usedBy:[40733,43320,45338],children:[]},{kind:"FunctionDeclaration",label:"function tickForMeasure",start:36356,end:36716,uses:[0,35544],usedBy:[40733],children:[]},{kind:"FunctionDeclaration",label:"function minimumCostAssignment",start:36718,end:38639,uses:[],usedBy:[39423],children:[]},{kind:"InterfaceDeclaration",label:"interface MarcherSquad",start:38641,end:38699,uses:[],usedBy:[38701,38927,39423],children:[]},{kind:"VariableStatement",label:"variable marcherSquads",start:38701,end:38925,uses:[38641],usedBy:[39423],children:[]},{kind:"FunctionDeclaration",label:"function squadLineCost",start:38927,end:39421,uses:[0,38641],usedBy:[39423],children:[]},{kind:"FunctionDeclaration",label:"function assignFormationSlots",start:39423,end:40731,uses:[0,38701,38641,38927,36718],usedBy:[40733],children:[]},{kind:"FunctionDeclaration",label:"function prepareDrill",start:40733,end:42719,uses:[3040,35572,35720,35830,34277,35647,36356,53406,63701,35683,39423,0,56410,61145,62587],usedBy:[44144],children:[]},{kind:"FunctionDeclaration",label:"function scheduleMidi",start:42721,end:43318,uses:[0,35433,35467,35544],usedBy:[43320],children:[]},{kind:"FunctionDeclaration",label:"function loadMidiData",start:43320,end:44142,uses:[0,35572,35608,42721,35683,35720,35830,34277,35647,4658,4498,4578,75795,35544],usedBy:[81209,0],children:[]},{kind:"FunctionDeclaration",label:"function loadDrillData",start:44144,end:44309,uses:[3040,40733,75795,35647,35683],usedBy:[49789],children:[]},{kind:"FunctionDeclaration",label:"function finiteNumber",start:44311,end:44519,uses:[],usedBy:[44521,45338,67065],children:[]},{kind:"FunctionDeclaration",label:"function validateCompiledMovement",start:44521,end:45336,uses:[0,44311],usedBy:[45338],children:[]},{kind:"FunctionDeclaration",label:"function prepareCompiledDrill",start:45338,end:48573,uses:[0,35572,44311,44521,35720,70260,35683,35647,35830,35433,35467,4658,4498,75795],usedBy:[48575,49476,49789],children:[]},{kind:"FunctionDeclaration",label:"function prepareCalChartShw",start:48575,end:49474,uses:[0,4882,45338,75795],usedBy:[81623,95012],children:[]},{kind:"FunctionDeclaration",label:"function prepareOpenMarch",start:49476,end:49787,uses:[0,45338,75795,35647],usedBy:[81948,95012],children:[]},{kind:"FunctionDeclaration",label:"function loadDrillFile",start:49789,end:49970,uses:[0,45338,44144,3040],usedBy:[82141],children:[]},{kind:"VariableStatement",label:"variable alphabetGlyphs",start:49972,end:51853,uses:[0],usedBy:[52713],children:[]},{kind:"FunctionDeclaration",label:"function samplePolyline",start:51855,end:52711,uses:[0],usedBy:[53406],children:[]},{kind:"FunctionDeclaration",label:"function alphabetPositions",start:52713,end:53404,uses:[0,49972],usedBy:[53406],children:[]},{kind:"FunctionDeclaration",label:"function formationPositions",start:53406,end:56371,uses:[0,52713,51855,5610,5654],usedBy:[40733,64805],children:[]},{kind:"VariableStatement",label:"variable minimumSweptClearance",start:56373,end:56408,uses:[],usedBy:[59659,60762],children:[]},{kind:"FunctionDeclaration",label:"function precalculateCollisionSafePath",start:56410,end:59551,uses:[0,63981,60762],usedBy:[40733],children:[]},{kind:"InterfaceDeclaration",label:"interface SweptCollision",start:59553,end:59657,uses:[],usedBy:[59659,60762],children:[]},{kind:"FunctionDeclaration",label:"function firstSweptCollision",start:59659,end:60760,uses:[0,59553,56373],usedBy:[60762],children:[]},{kind:"FunctionDeclaration",label:"function reportPathCollision",start:60762,end:61143,uses:[0,59659,59553,56373],usedBy:[56410],children:[]},{kind:"FunctionDeclaration",label:"function precalculatePathProgress",start:61145,end:61900,uses:[0],usedBy:[40733],children:[]},{kind:"FunctionDeclaration",label:"function samplePrecalculatedPath",start:61902,end:62585,uses:[0],usedBy:[62587,64805],children:[]},{kind:"FunctionDeclaration",label:"function precalculateFootsteps",start:62587,end:63699,uses:[0,2505,61902],usedBy:[40733],children:[]},{kind:"FunctionDeclaration",label:"function facingAngle",start:63701,end:63979,uses:[],usedBy:[40733,64805],children:[]},{kind:"FunctionDeclaration",label:"function interpolateAngle",start:63981,end:64166,uses:[],usedBy:[56410,64168,64805],children:[]},{kind:"FunctionDeclaration",label:"function smoothAngle",start:64168,end:64321,uses:[63981],usedBy:[93763],children:[]},{kind:"InterfaceDeclaration",label:"interface DrillMotion",start:64323,end:64409,uses:[0],usedBy:[64411,64616,67065,83780,84593,89936],children:[]},{kind:"InterfaceDeclaration",label:"interface CompiledMovementSample",start:64411,end:64527,uses:[0,64323],usedBy:[67065,70145,70883],children:[]},{kind:"InterfaceDeclaration",label:"interface DrillDisplayCue",start:64529,end:64614,uses:[0],usedBy:[64616,72827],children:[]},{kind:"InterfaceDeclaration",label:"interface DrillPlaybackState",start:64616,end:64803,uses:[64529,0,64323],usedBy:[64805,70202,70883,72648,72827,93566,93677,93722,93763,93915],children:[]},{kind:"FunctionDeclaration",label:"function nativeDrillState",start:64805,end:66750,uses:[64616,35683,0,53406,34277,63701,61902,35572,2505,63981,3040],usedBy:[72648],children:[]},{kind:"FunctionDeclaration",label:"function fieldGridPosition",start:66752,end:66894,uses:[0,5513],usedBy:[67065],children:[]},{kind:"FunctionDeclaration",label:"function fieldGridFacing",start:66896,end:67063,uses:[],usedBy:[67065],children:[]},{kind:"FunctionDeclaration",label:"function sampleCompiledMovement",start:67065,end:70143,uses:[0,64411,66752,44311,66896,64323,5513],usedBy:[70883],children:[]},{kind:"VariableStatement",label:"variable compiledFrameSamples",start:70145,end:70201,uses:[64411],usedBy:[70883],children:[]},{kind:"VariableStatement",label:"variable compiledFrameState",start:70202,end:70259,uses:[64616],usedBy:[70883],children:[]},{kind:"VariableStatement",label:"variable compiledFrameSetIndex",start:70260,end:70291,uses:[],usedBy:[45338,70883],children:[]},{kind:"FunctionDeclaration",label:"function tempoMapBeat",start:70293,end:70881,uses:[3415,35572],usedBy:[70883,93094],children:[]},{kind:"FunctionDeclaration",label:"function compiledDrillState",start:70883,end:72646,uses:[3415,64616,70293,0,70202,70145,64411,70260,3926,67065],usedBy:[72648],children:[]},{kind:"FunctionDeclaration",label:"function currentDrillState",start:72648,end:72825,uses:[64616,35720,70883,64805],usedBy:[92954],children:[]},{kind:"FunctionDeclaration",label:"function drawScoreboard",start:72827,end:73975,uses:[64616,35647,64529,35797,34325],usedBy:[0],children:[]},{kind:"FunctionDeclaration",label:"function configureFieldScale",start:73977,end:74510,uses:[74642,5396,5423,5873,34244,5796,7084],usedBy:[76867,77659,0],children:[]},{kind:"VariableStatement",label:"variable xrExperience",start:74512,end:74567,uses:[0],usedBy:[74983,75052,75881,76275,76867,77659],children:[]},{kind:"VariableStatement",label:"variable requestingXr",start:74568,end:74593,uses:[],usedBy:[75881,76867],children:[]},{kind:"VariableStatement",label:"variable supportsAr",start:74594,end:74617,uses:[],usedBy:[75881,77659],children:[]},{kind:"VariableStatement",label:"variable supportsVr",start:74618,end:74641,uses:[],usedBy:[75881,77659],children:[]},{kind:"VariableStatement",label:"variable activeDisplayMode",start:74642,end:74721,uses:[],usedBy:[73977,74983,93355,93462,93892,94299],children:[]},{kind:"VariableStatement",label:"variable vrFlightAxes",start:74722,end:74758,uses:[],usedBy:[75107,75191,77659,79868],children:[]},{kind:"VariableStatement",label:"variable vrFlightForward",start:74759,end:74798,uses:[0],usedBy:[0],children:[]},{kind:"VariableStatement",label:"variable vrFlightRight",start:74799,end:74836,uses:[0],usedBy:[0],children:[]},{kind:"VariableStatement",label:"variable vrFlightMovement",start:74837,end:74877,uses:[0],usedBy:[0,75575],children:[]},{kind:"VariableStatement",label:"variable vrFlightDeadZone",start:74878,end:74908,uses:[],usedBy:[75107,75191],children:[]},{kind:"VariableStatement",label:"variable vrFlightSpeed",start:74909,end:74936,uses:[],usedBy:[0],children:[]},{kind:"IfStatement",label:"IfStatement",start:74983,end:75049,uses:[74642,74512],usedBy:[],children:[{kind:"ReturnStatement",label:"ReturnStatement",start:75042,end:75049,uses:[],usedBy:[],children:[]}]},{kind:"VariableStatement",label:"variable xrCamera",start:75052,end:75104,uses:[74512],usedBy:[0],children:[]},{kind:"VariableStatement",label:"variable strafe",start:75107,end:75188,uses:[74722,74878],usedBy:[75277,0],children:[]},{kind:"VariableStatement",label:"variable forward",start:75191,end:75274,uses:[74722,74878],usedBy:[75277,0],children:[]},{kind:"IfStatement",label:"IfStatement",start:75277,end:75319,uses:[75107,75191],usedBy:[],children:[{kind:"ReturnStatement",label:"ReturnStatement",start:75312,end:75319,uses:[],usedBy:[],children:[]}]},{kind:"IfStatement",label:"IfStatement",start:75575,end:75646,uses:[74837],usedBy:[],children:[]},{kind:"FunctionDeclaration",label:"function setStatus",start:75795,end:75879,uses:[4004],usedBy:[43320,44144,45338,48575,49476,76867,77659,81209,81541,81825,0],children:[]},{kind:"FunctionDeclaration",label:"function updateButtons",start:75881,end:76273,uses:[74512,0,4162,74568,74594,4249,74618],usedBy:[76867,77659],children:[]},{kind:"FunctionDeclaration",label:"function configureXrInteraction",start:76275,end:76865,uses:[74512,34735,34868,34371,5873],usedBy:[76867,77659],children:[]},{kind:"FunctionDeclaration",label:"function enterXr",start:76867,end:77657,uses:[74512,74568,73977,76275,75881,75795],usedBy:[0],children:[]},{kind:"FunctionDeclaration",label:"function initializeXr",start:77659,end:79149,uses:[74512,5137,34118,79868,74722,74594,74618,0,73977,76275,75881,75795],usedBy:[0],children:[]},{kind:"FunctionDeclaration",label:"function togglePlayback",start:79151,end:79506,uses:[35572,0,35433,35544,4498],usedBy:[27090,79868,0],children:[]},{kind:"FunctionDeclaration",label:"function stopPlayback",start:79508,end:79643,uses:[35433,35467,4498],usedBy:[27090,79868,0],children:[]},{kind:"FunctionDeclaration",label:"function seekPlayback",start:79645,end:79866,uses:[35572,35433,35544,35467],usedBy:[27090,79868],children:[]},{kind:"FunctionDeclaration",label:"function initializeQuestController",start:79868,end:80961,uses:[0,74722,79151,79508,79645],usedBy:[77659],children:[]},{kind:"VariableStatement",label:"variable file",start:81168,end:81206,uses:[4336],usedBy:[81209],children:[]},{kind:"IfStatement",label:"IfStatement",start:81209,end:81377,uses:[81168,43320,75795],usedBy:[],children:[]},{kind:"VariableStatement",label:"variable file",start:81434,end:81473,uses:[4416],usedBy:[81476,81541,81623,81825,0],children:[]},{kind:"IfStatement",label:"IfStatement",start:81476,end:81494,uses:[81434],usedBy:[],children:[{kind:"ReturnStatement",label:"ReturnStatement",start:81487,end:81494,uses:[],usedBy:[],children:[]}]},{kind:"VariableStatement",label:"variable generation",start:81497,end:81538,uses:[35768],usedBy:[81623,81908,82141],children:[]},{kind:"IfStatement",label:"IfStatement",start:81541,end:81822,uses:[81434,75795],usedBy:[81623],children:[{kind:"IfStatement",label:"IfStatement",start:81623,end:81699,uses:[81497,35768,48575,81541,81434],usedBy:[],children:[]},{kind:"ReturnStatement",label:"ReturnStatement",start:81811,end:81818,uses:[],usedBy:[],children:[]}]},{kind:"IfStatement",label:"IfStatement",start:81825,end:82102,uses:[81434,75795],usedBy:[81948],children:[{kind:"IfStatement",label:"IfStatement",start:81908,end:81978,uses:[81497,35768],usedBy:[],children:[{kind:"ReturnStatement",label:"ReturnStatement",start:81948,end:81978,uses:[49476,81825],usedBy:[],children:[]}]},{kind:"ReturnStatement",label:"ReturnStatement",start:82091,end:82098,uses:[],usedBy:[],children:[]}]},{kind:"IfStatement",label:"IfStatement",start:82141,end:82213,uses:[81497,35768,49789,0],usedBy:[],children:[]},{kind:"IfStatement",label:"IfStatement",start:82489,end:82513,uses:[35572],usedBy:[],children:[{kind:"ReturnStatement",label:"ReturnStatement",start:82506,end:82513,uses:[],usedBy:[],children:[]}]},{kind:"VariableStatement",label:"variable gaitProfiles",start:82747,end:83778,uses:[0],usedBy:[84593],children:[]},{kind:"FunctionDeclaration",label:"function travelGait",start:83780,end:84443,uses:[0,64323],usedBy:[93915],children:[]},{kind:"FunctionDeclaration",label:"function stationaryLegLifts",start:84445,end:84591,uses:[],usedBy:[84593,89936],children:[]},{kind:"FunctionDeclaration",label:"function animatePerformerRig",start:84593,end:89934,uses:[0,64323,2664,82747,5513,84445,5049,5729,5763],usedBy:[93892],children:[]},{kind:"FunctionDeclaration",label:"function animateSimplifiedPerformerRig",start:89936,end:92478,uses:[0,64323,2664,84445,5729,5763],usedBy:[93892],children:[]},{kind:"VariableStatement",label:"variable arRigUpdateInterval",start:92480,end:92515,uses:[],usedBy:[92516,93355,93462],children:[]},{kind:"VariableStatement",label:"variable arRigUpdateAccumulator",start:92516,end:92565,uses:[92480],usedBy:[0,93355,93462],children:[]},{kind:"VariableStatement",label:"variable deltaSeconds",start:92612,end:92662,uses:[5049],usedBy:[0,93763],children:[]},{kind:"IfStatement",label:"IfStatement",start:92665,end:92771,uses:[7084,5137],usedBy:[],children:[]},{kind:"VariableStatement",label:"variable seconds",start:92774,end:92875,uses:[35572,35433,35544],usedBy:[92878,92954,93094,94299],children:[]},{kind:"VariableStatement",label:"variable ticks",start:92878,end:92951,uses:[35572,92774],usedBy:[92954,93094],children:[]},{kind:"VariableStatement",label:"variable drill",start:92954,end:93002,uses:[72648,92878,92774],usedBy:[0,93566,93677,93722,93763,93915],children:[]},{kind:"VariableStatement",label:"variable stepTicks",start:93030,end:93091,uses:[35572],usedBy:[93094],children:[]},{kind:"VariableStatement",label:"variable stepPhase",start:93094,end:93260,uses:[35720,70293,92774,3415,92878,93030],usedBy:[93892],children:[]},{kind:"VariableStatement",label:"variable marching",start:93263,end:93310,uses:[35433],usedBy:[93892],children:[]},{kind:"VariableStatement",label:"variable updateRig",start:93355,end:93459,uses:[74642,92516,92480],usedBy:[93462,93892],children:[]},{kind:"IfStatement",label:"IfStatement",start:93462,end:93563,uses:[93355,74642,92516,92480],usedBy:[],children:[]},{kind:"ForStatement",label:"ForStatement",start:93566,end:94296,uses:[92954,64616,93636,93677,93763],usedBy:[93636,93677,93722,93763,93915],children:[{kind:"VariableStatement",label:"variable performer",start:93636,end:93672,uses:[34244,93566],usedBy:[93763,93566,93892],children:[]},{kind:"VariableStatement",label:"variable position",start:93677,end:93717,uses:[92954,64616,93566],usedBy:[93566],children:[]},{kind:"VariableStatement",label:"variable motion",start:93722,end:93758,uses:[92954,64616,93566],usedBy:[93915,93892],children:[]},{kind:"VariableStatement",label:"variable facing",start:93763,end:93848,uses:[64168,93636,92954,64616,93566,92612],usedBy:[93915,93892,93566],children:[]},{kind:"IfStatement",label:"IfStatement",start:93892,end:94218,uses:[93355,93636,74642,89936,93094,93915,93722,93263,84593,93763],usedBy:[],children:[{kind:"VariableStatement",label:"variable gait",start:93915,end:93975,uses:[83780,92954,64616,93566,93722,93763],usedBy:[93892],children:[]}]}]},{kind:"IfStatement",label:"IfStatement",start:94299,end:94446,uses:[35572,74642,4658,92774,35544],usedBy:[],children:[]},{kind:"VariableStatement",label:"variable defaultDrillGeneration",start:94554,end:94605,uses:[35768],usedBy:[94948],children:[]},{kind:"IfStatement",label:"IfStatement",start:94948,end:95007,uses:[94554,35768],usedBy:[],children:[{kind:"ReturnStatement",label:"ReturnStatement",start:95e3,end:95007,uses:[],usedBy:[],children:[]}]},{kind:"IfStatement",label:"IfStatement",start:95012,end:95139,uses:[4882,49476,0,48575],usedBy:[],children:[]}]},Be=document.getElementById("build-info"),Pe=document.getElementById("status"),ze=document.getElementById("render-canvas"),re=document.getElementById("enter-ar-button"),ie=document.getElementById("enter-vr-button"),Ie=document.getElementById("controller-palette");function x(e){Pe&&(Pe.textContent=e)}function fe(){const e=L?.baseExperience.state===G.IN_XR;re&&(re.textContent=e?"Quest 3 AR Active":"Enter Quest 3 AR",re.disabled=O||e||!ue),ie&&(ie.textContent=e?"VR Active":"Enter VR",ie.disabled=O||e||!ve)}function rn(){if(!Be)return;const e=Number(xe.buildNumber),n=`${xe.name} v${xe.version} · build ${e} · production`;Be.textContent=n}Ve.UseOnlineRepository=!0;Ve.PrioritizeOnlineRepository=!0;const we=new Ye(ze,!0,{adaptToDeviceRatio:!0,antialias:!0}),b=new Qe(we);b.clearColor=new pe(0,0,0,0);const $e=new Ke("cam",-Math.PI/2,Math.PI/3,3,l.Zero(),b);$e.attachControl(ze,!1);$e.inputs.clear();const on=new en("light",new l(0,1,0),b);on.intensity=.9;const j=new Le("sceneRoot",b),T=[],se=new Map,he=new Map,Xe=new WeakSet,qe=new WeakMap;let N=null;const oe=[],ge=new Map,H=new Map,J=new Map,le=new Map,de=new Set,Q=new Map;let A=null,I=null,B=null,ce=null;const E=new Map,X=new Map,K=[];let S=null,U=null,ee=null,F=null,be=!1,L=null,O=!1,ue=!1,ve=!1;function sn(e){Ie&&Ie.classList.toggle("visible",e)}function _(e){if(!e){for(const[a,r]of X)Me(a,r.diffuseColor,r.emissiveColor);E.clear(),X.clear();return}const n=E.get(e);if(!n)return;E.delete(e);const t=X.get(n);t&&(t.owners.delete(e),!(t.owners.size>0)&&(Me(n,t.diffuseColor,t.emissiveColor),X.delete(n)))}function Me(e,n,t){if(Xe.has(e)){e.instancedBuffers.color=new pe(n.r,n.g,n.b,1);return}const a=e.material;a?.diffuseColor.copyFrom(n),a?.emissiveColor.copyFrom(t)}function Oe(e){const n=X.get(e);if(!n)return;const t=Array.from(n.owners).some(a=>(Q.get(a)??!1)||de.has(a));Me(e,t?new y(.25,.7,.7):new y(.18,.42,.42),t?new y(.35,1,1):new y(.08,.35,.32))}function ln(e,n){if(n){const r=E.get(e);r&&r!==n&&_(e)}else{_(e);return}const t=n.material;if(!t)return;let a=X.get(n);if(!a){const r=qe.get(n)??t.diffuseColor;a={emissiveColor:t.emissiveColor.clone(),diffuseColor:r.clone(),owners:new Set},X.set(n,a)}a.owners.add(e),E.set(e,n),Oe(n)}function ye(){if(We(),T.length>0)return;const e=Pn(an);x(`🧠 AST scene built (${e} nodes)`)}const dn=new Set(["as","async","await","break","case","catch","class","const","continue","default","delete","do","else","export","extends","false","finally","for","from","function","if","implements","import","in","instanceof","interface","let","new","null","of","private","protected","public","readonly","return","static","super","switch","this","throw","true","try","type","typeof","undefined","void","while","with","yield"]);function cn(e){let n=!1;return e.map(t=>{const a=[];let r=0;for(;r<t.length;){if(n){const m=t.indexOf("*/",r),d=m<0?t.length:m+2;a.push({text:t.slice(r,d),kind:"comment"}),r=d,n=m<0;continue}if(t.startsWith("//",r)){a.push({text:t.slice(r),kind:"comment"});break}if(t.startsWith("/*",r)){const m=t.indexOf("*/",r+2),d=m<0?t.length:m+2;a.push({text:t.slice(r,d),kind:"comment"}),r=d,n=m<0;continue}const s=t.slice(r),u=s.match(/^(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|`(?:\\.|[^`\\])*`)/),o=s.match(/^(?:0[xob][\da-f]+|\d+(?:\.\d+)?(?:e[+-]?\d+)?)/i),i=s.match(/^[A-Za-z_$][\w$]*/),f=s.match(/^\s+/);if(u)a.push({text:u[0],kind:"string"});else if(o)a.push({text:o[0],kind:"number"});else if(i){const m=i[0],d=dn.has(m)?"keyword":/^[A-Z]/.test(m)?"type":"plain";a.push({text:m,kind:d})}else f?a.push({text:f[0],kind:"plain"}):a.push({text:s[0],kind:/[=+\-*/%!?<>|&:^~]/.test(s[0])?"operator":"plain"});r+=a[a.length-1].text.length}return a})}function Ge(e){if(!ee)return;const n=ee.getContext();n.fillStyle="#06151b",n.fillRect(0,0,1024,576);const t=e?.metadata,a=t?.sourceStart!==void 0&&t.sourceEnd!==void 0,r=Se.replace(/\t/g,"  ").split(/\r?\n/),s=cn(r),u=a?Se.slice(0,t.sourceStart).split(`
`).length:1,o=a?Se.slice(0,t.sourceEnd).split(`
`).length:0,i=14,f=Math.max(1,Math.min(u-4,r.length-i+1)),m=r.slice(f-1,f-1+i),d=f+m.length-1,c=t?.astLabel??"chart-xr.ts",g=t?.astKind??(e?"Manual node - showing complete source":"Complete program source");n.fillStyle="#72f0ff",n.font="bold 30px monospace",n.fillText(c.slice(0,52),38,48),n.fillStyle="#6f91a0",n.font="22px monospace",n.fillText(g,38,80);const v=`${f}-${d} / ${r.length}`;n.fillText(v,986-n.measureText(v).width,80),n.fillStyle="#17404a",n.fillRect(38,96,948,2),n.font="24px monospace",m.forEach((k,P)=>{const M=f+P,D=130+P*30,z=a&&M>=u&&M<=o;z&&(n.fillStyle="#123b44",n.fillRect(30,D-23,956,29)),n.fillStyle=z?"#72f0ff":"#4d7582",n.fillText(String(M).padStart(4," "),38,D);let p=118,h=70;for(const C of s[M-1]){if(h<=0)break;const w=C.text.slice(0,h),R={plain:"#d8e7ec",keyword:"#ff78bd",type:"#72e6c1",string:"#ffd580",number:"#b8a0ff",comment:"#668995",operator:"#72d9ff"};n.fillStyle=R[C.kind],n.fillText(w,p,D),p+=n.measureText(w).width,h-=w.length}}),ee.update()}function We(){if(S)return;S=new Le("source-tablet",b),S.parent=j,S.position=new l(.42,.28,.38),S.rotationQuaternion=q.RotationYawPitchRoll(Math.PI,0,0);const e=new W("tablet-frame-material",b);e.diffuseColor=new y(.035,.045,.06),e.emissiveColor=new y(.008,.012,.02);const n=new W("tablet-screen-material",b);n.diffuseColor=new y(.02,.08,.1),n.emissiveColor=new y(.02,.45,.55),n.disableLighting=!0,U=V.CreateBox("tablet-body",{width:.46,height:.29,depth:.022},b),U.parent=S,U.material=e,U.isPickable=!0;const t=V.CreatePlane("tablet-screen",{width:.42,height:.245},b);t.parent=S,t.position.z=.012,t.rotation.y=Math.PI,t.material=n,t.isPickable=!1;const a=V.CreateBox("tablet-handle",{width:.16,height:.025,depth:.035},b);a.parent=S,a.position.y=-.165,a.material=e,a.isPickable=!1,ee=new Ee("tablet-screen-texture",{width:1024,height:576},b,!0),Ge(null),n.diffuseTexture=ee,n.emissiveTexture=ee}function De(e){return e==="SourceFile"?new y(.82,.88,.95):e==="FunctionDeclaration"?new y(.1,.72,.9):e==="ClassDeclaration"?new y(.95,.55,.15):e==="InterfaceDeclaration"?new y(.2,.78,.42):e==="VariableStatement"?new y(.35,.48,.95):e==="ReturnStatement"?new y(.9,.3,.65):e.endsWith("Statement")?new y(.95,.3,.25):new y(.55,.62,.72)}function un(e,n,t=.075,a=!1){const r=document.createElement("canvas").getContext("2d"),s=48;r&&(r.font=`bold ${s}px sans-serif`);const u=r?.measureText(n).width??n.length*s*.6,o=Math.max(12,Math.floor(s*Math.min(1,2e3/Math.max(u,1)))),i=`bold ${o}px sans-serif`;r&&(r.font=i);const m=(r?.measureText(n).width??n.length*o*.6)+48,d=Math.min(2048,Math.max(256,2**Math.ceil(Math.log2(m)))),c=128,v=.06*d/c,k=V.CreatePlane(`${e.name}_label`,{width:v,height:.06},b);k.parent=e,k.position.y=t,k.billboardMode=tn.BILLBOARDMODE_ALL,k.isPickable=a,a&&he.set(k,e);const P=new Ee(`${e.name}_label_texture`,{width:d,height:c},b,!0);P.hasAlpha=!0,P.drawText(n,null,84,i,"#ffffff","rgba(7, 17, 31, 0.88)",!0,!0);const M=new W(`${e.name}_label_material`,b);M.diffuseTexture=P,M.emissiveTexture=P,M.opacityTexture=P,M.disableLighting=!0,M.backFaceCulling=!1,k.material=M,k.onDisposeObservable.add(()=>{he.delete(k),M.dispose(!1,!0)})}function _e(e,n,t="ManualNode",a=new l(.08,.08,.08),r=1){const s=`box_${T.length}`,u=t==="VariableStatement"&&r===1;let o;if(u){if(!N){N=V.CreatePolyhedron("variable-octahedron-source",{type:1,size:a.x/(2*Math.SQRT2),flat:!0},b),N.position.y=-1e3,N.isPickable=!1,N.useVertexColors=!0,N.registerInstancedBuffer("color",4),N.instancedBuffers.color=new pe(1,1,1,1);const f=new W("variable-octahedron-material",b);f.diffuseColor=y.White(),f.emissiveColor=new y(.094,.094,.094),N.material=f}o=N.createInstance(s);const i=De(t);Xe.add(o),qe.set(o,i),o.instancedBuffers.color=new pe(i.r,i.g,i.b,1)}else{o=V.CreateBox(s,{width:a.x,height:a.y,depth:a.z},b);const i=new W(`boxMat_${T.length}`,b);i.diffuseColor=De(t),i.emissiveColor=new y(.094,.094,.094),i.alpha=r,i.backFaceCulling=r===1,o.material=i}return o.position.copyFrom(e),o.rotationQuaternion=q.Identity(),o.parent=j,o.isPickable=r===1,o.computeWorldMatrix(!0),T.push(o),un(o,n??`Node ${T.length}`,a.y/2+.035,r<1),o}function Te(e,n){const t=n.position.subtract(e.position),a=e.getBoundingInfo().boundingBox.extendSize,r=e.position.clone();return Math.abs(t.x)>=Math.abs(t.y)&&Math.abs(t.x)>=Math.abs(t.z)?r.x+=Math.sign(t.x)*a.x:Math.abs(t.y)>=Math.abs(t.z)?r.y+=Math.sign(t.y)*a.y:r.z+=Math.sign(t.z)*a.z,r}function mn(e,n){const t=se.get(e);if(!t)return n;const a=t.getBoundingInfo().boundingBox.extendSize,r=e.getBoundingInfo().boundingBox.extendSize,s=.03,u=new l(Math.max(0,a.x-r.x-s),Math.max(0,a.y-r.y-s),Math.max(0,a.z-r.z-s)),o=t.position.subtract(u),i=t.position.add(u);return new l(Math.max(o.x,Math.min(n.x,i.x)),Math.max(o.y,Math.min(n.y,i.y)),Math.max(o.z,Math.min(n.z,i.z)))}function pn(e,n){const t=mn(e,n),a=t.subtract(e.position);if(!(a.lengthSquared()<1e-12)){e.position.copyFrom(t),e.computeWorldMatrix(!0);for(const r of T){let s=se.get(r);for(;s;){if(s===e){r.position.addInPlace(a),r.computeWorldMatrix(!0);break}s=se.get(s)}}}}function Ze(){for(const e of oe){const n=Te(e.startNode,e.endNode),t=Te(e.endNode,e.startNode),a=t.subtract(n),r=Math.max(a.length(),1e-4),s=a.scale(1/r),u=.12,o=Math.min(.08,r*.4),i=Math.max(r-o,.001),f=Math.max(.021,e.weight*.03),m=n.add(s.scale(i/2));e.shaft.position.copyFrom(m),e.shaft.scaling=new l(f,i,f);const d=l.Up(),c=l.Cross(d,s),g=Math.acos(Math.max(-1,Math.min(1,l.Dot(d,s))));e.shaft.rotationQuaternion=c.lengthSquared()<1e-6?q.Identity():q.RotationAxis(c.normalize(),g);const v=t.subtract(s.scale(o/2)),k=Math.max(.016,f*2.4);e.head.position.copyFrom(v),e.head.scaling=new l(k,o/u,k),e.head.rotationQuaternion=e.shaft.rotationQuaternion?.clone()??q.Identity()}}function fn(e){for(const[a,r]of Array.from(E.entries()))r===e&&_(a);const n=T.indexOf(e);n>=0&&T.splice(n,1),se.delete(e);const t=oe.filter(a=>a.startNode===e||a.endNode===e);for(const a of t)a.shaft.dispose(),a.head.dispose();for(const a of t){const r=oe.indexOf(a);r>=0&&oe.splice(r,1)}e.dispose()}function He(e){return e.inputSource.handedness==="left"?"left":"right"}function Ue(e){const n=e.motionController;if(!n)return{x:0,y:0};const t=n.getComponent("thumbstick")??n.getComponent("xr-standard-thumbstick")??n.getComponentOfType("thumbstick")??n.getComponent("touchpad");if(!t)return{x:0,y:0};const a=t.axes,r=a?.x??0,s=a?.y??0;return{x:Math.abs(r)<.2?0:r,y:Math.abs(s)<.2?0:s}}function hn(e){return-Ue(e).y*.02}function gn(e){const n=b.activeCamera;if(!n)return;const t=n.getDirection(l.Forward());if(t.y=0,t.lengthSquared()<1e-6)return;t.normalize();const a=l.Cross(t,l.Up()).normalize(),r=l.Zero(),s=.8,u=1.2;let o=0;for(const d of K){if(de.has(d)||J.has(d))continue;const c=Ue(d);He(d)==="left"?(r.addInPlace(a.scale(-c.x)),r.addInPlace(t.scale(c.y))):(r.addInPlace(l.Up().scale(c.y)),o-=c.x*u*e)}r.scaleInPlace(s*e);const i=n.globalPosition,f=Math.cos(o),m=Math.sin(o);for(const d of T){if(d.position.addInPlace(r),Math.abs(o)>1e-6){const c=d.position.x-i.x,g=d.position.z-i.z;d.position.x=i.x+c*f+g*m,d.position.z=i.z-c*m+g*f}d.computeWorldMatrix(!0)}if(S&&!F){if(S.position.addInPlace(r),Math.abs(o)>1e-6){const d=S.position.x-i.x,c=S.position.z-i.z;S.position.x=i.x+d*f+c*m,S.position.z=i.z-d*m+c*f,S.rotate(l.Up(),-o)}S.computeWorldMatrix(!0)}}function bn(e){const a=((e.pointer??e.grip)?.getDirection?.(l.Forward())??l.Forward()).normalize();let r=l.Cross(a,l.Up());r.lengthSquared()<1e-6&&(r=l.Cross(a,l.Forward())),r.normalize();const s=l.Cross(r,a);return s.normalize(),{depthAxis:a,rightAxis:r,upAxis:s}}function Y(e,n=!0,t=!1){if(n){const m=H.get(e);if(m)return m}if(L?.baseExperience.state===G.IN_XR&&!e._lastXRPose){const m={mesh:null,point:null};return n&&H.set(e,m),m}const a=e.pointer??e.grip;if(!a)return null;const r=a.absolutePosition?.clone()??l.Zero(),u=(a.getDirection?.(l.Forward())??l.Forward()).normalize();let o=l.Cross(u,l.Up());o.lengthSquared()<1e-6&&(o=l.Cross(u,l.Forward())),o.normalize();const i=[u,u.add(o.scale(.08)).normalize(),u.add(o.scale(-.08)).normalize(),u.add(o.scale(.16)).normalize(),u.add(o.scale(-.16)).normalize()];for(const m of i){const d=new Ne(r,m);d.length=8;const c=b.pickWithRay(d,g=>T.includes(g)||t&&he.has(g));if(c?.pickedMesh){const v={mesh:he.get(c.pickedMesh)??c.pickedMesh,point:c.pickedPoint?.clone()??r.add(m.scale(.8))};return n&&H.set(e,v),v}}const f={mesh:null,point:null};return n&&H.set(e,f),f}function ke(e,n=!1){return Y(e,!n,n)?.mesh??null}function yn(e){const n=[e.pointer?.absolutePosition,e.grip?.absolutePosition].filter(s=>!!s);if(n.length===0)return null;const t=.025**2;let a=null,r=Number.POSITIVE_INFINITY;for(const s of T){if(!s.isPickable)continue;s.computeWorldMatrix(!0);const u=s.getBoundingInfo().boundingBox;for(const o of n){const i=Math.max(u.minimumWorld.x,Math.min(o.x,u.maximumWorld.x)),f=Math.max(u.minimumWorld.y,Math.min(o.y,u.maximumWorld.y)),m=Math.max(u.minimumWorld.z,Math.min(o.z,u.maximumWorld.z)),d=l.DistanceSquared(o,new l(i,f,m));d<=t&&d<r&&(a=s,r=d)}}return a}function Fe(e){if(!U||F&&F!==e)return!1;const n=e.pointer??e.grip;if(!n)return!1;const t=n.absolutePosition?.clone()??l.Zero(),a=n.getDirection?.(l.Forward()).normalize()??l.Forward(),r=new Ne(t,a,8);return b.pickWithRay(r,s=>s===U)?.pickedMesh===U}function xn(){if(!S||!F)return;const e=F.grip??F.pointer,n=b.activeCamera;!e?.absolutePosition||!n||(S.position.copyFrom(e.absolutePosition.add(l.Up().scale(.09))),S.lookAt(n.globalPosition),S.computeWorldMatrix(!0))}function me(){S||We();const e=b.activeCamera;if(!S||!e)return;F=null;const n=e.getDirection(l.Forward()).normalize();S.position.copyFrom(e.globalPosition.add(n.scale(.65)).add(l.Down().scale(.12))),S.lookAt(e.globalPosition),S.computeWorldMatrix(!0),x("📟 Tablet summoned")}function Sn(e){const n=V.CreateCylinder(`beam_${e.uniqueId??"controller"}`,{height:1,diameter:.008,tessellation:6},b);n.parent=j,n.isPickable=!1;const t=new W(`beamMat_${e.uniqueId??"controller"}`,b);return t.diffuseColor=new y(.2,.8,1),t.emissiveColor=new y(.1,.35,.7),t.alpha=.9,t.disableLighting=!0,n.material=t,n}function vn(e,n){const t=e.pointer??e.grip,a=ge.get(e)??Sn(e);if(ge.set(e,a),!t){a.setEnabled(!1);return}const r=t.absolutePosition?.clone()??l.Zero(),u=(t.getDirection?.(l.Forward())??l.Forward()).normalize(),o=n??Y(e),i=J.get(e)??null,f=i?i.position.clone():o?.point,m=f?l.Distance(r,f):2.2;if(a.setEnabled(!0),a.position.copyFrom(r.add(u.scale(m/2))),a.scaling.y=m,a.scaling.x=1,a.scaling.z=1,u.lengthSquared()>1e-6){const d=l.Up(),c=l.Cross(d,u),g=Math.acos(Math.max(-1,Math.min(1,l.Dot(d,u))));a.rotationQuaternion=c.lengthSquared()<1e-6?q.Identity():q.RotationAxis(c.normalize(),g)}else a.rotationQuaternion=q.Identity()}function Mn(e,n){const t=e.pointer??e.grip,r=(n??Y(e))?.mesh??null;if(!t){_(e);return}r?E.get(e)!==r&&ln(e,r):_(e)}function Ce(e){return K.filter(t=>t!==e)[0]??null}function Je(e){const n=[I===e?Ce(e):e,e].filter(t=>!!t);for(const t of n){const a=ke(t)??E.get(t)??null;if(a&&a!==A)return a}return null}function wn(e,n){const t=e.pointer??e.grip;if(!t)return l.Zero();const a=t.absolutePosition?.clone()??l.Zero(),s=(t.getDirection?.(l.Forward())??l.Forward()).normalize(),u=I===e?Ce(e):e,o=n??Y(u||e);return o?.point?o.point.clone():a.add(s.scale(.8))}function ne(){ce&&(ce.dispose(),ce=null)}function kn(e,n){if(!A||I!==e){B&&(B.dispose(),B=null);const o=n?.mesh??null;if(o){const i=n?.point?.clone()??e.pointer?.absolutePosition?.clone()??e.grip?.absolutePosition?.clone()??l.Zero(),f=o.position.clone();ne();const m=V.CreateLines("hover-connection-preview",{points:[i,f]},b);m.color=new y(.2,.85,1),m.alpha=.35,m.parent=j,m.isPickable=!1,ce=m}else ne();return}ne();const t=Ce(e),a=t?Y(t):null,s=(a?.mesh??null)?.position?.clone()??wn(t??e,a??void 0);B&&B.dispose();const u=V.CreateLines("connection-preview",{points:[A.position.clone(),s]},b);u.color=new y(.35,1,1),u.alpha=1,u.parent=j,u.isPickable=!1,B=u}function Cn(e){return e==="reference"?{diffuse:new y(1,.25,.82),emissive:new y(.9,.12,.65),head:new y(1,.42,.9)}:e==="user"?{diffuse:new y(1,.72,.18),emissive:new y(.95,.5,.08),head:new y(1,.84,.35)}:{diffuse:new y(.35,1,1),emissive:new y(.25,.9,1),head:new y(.45,1,1)}}function je(e,n,t=1,a="user"){const r=Cn(a),s=V.CreateCylinder("connection-shaft",{height:1,diameter:.04,tessellation:12},b);s.parent=j,s.isPickable=!1,s.metadata={connectionKind:a};const u=new W("connection-shaft-mat",b);u.diffuseColor=r.diffuse,u.emissiveColor=r.emissive,u.disableLighting=!0,s.material=u;const o=V.CreateCylinder("connection-head",{height:.12,diameterTop:.001,diameterBottom:.08,tessellation:12},b);o.parent=j,o.isPickable=!1,o.metadata={connectionKind:a};const i=new W("connection-head-mat",b);return i.diffuseColor=r.head,i.emissiveColor=r.emissive,i.disableLighting=!0,o.material=i,oe.push({shaft:s,head:o,startNode:e,endNode:n,weight:t,kind:a}),Ze(),s}class Bn{constructor(n,t,a,r){this.idealEdgeLength=n,this.depthSpacing=t,this.origin=a,this.iterations=r}layout(n,t){const a=Math.PI*(3-Math.sqrt(5)),r=n.map((o,i)=>{const f=this.idealEdgeLength*Math.sqrt(i+1)*.45,m=i*a;return new l(Math.cos(m)*f,Math.sin(m)*f*.25,-o.depth*this.depthSpacing)}),s=this.idealEdgeLength*this.idealEdgeLength;for(let o=0;o<this.iterations;o+=1){const i=n.map(()=>l.Zero());for(let d=0;d<r.length;d+=1)for(let c=d+1;c<r.length;c+=1){const g=r[d].subtract(r[c]),v=Math.max(g.length(),.001),k=g.scale(s/(v*v*v));i[d].addInPlace(k),i[c].subtractInPlace(k)}for(const d of t){const c=r[d.targetIndex].subtract(r[d.sourceIndex]),g=Math.max(c.length(),.001),v=g-this.idealEdgeLength,k=c.scale(v*d.strength/g);i[d.sourceIndex].addInPlace(k),i[d.targetIndex].subtractInPlace(k)}n.forEach((d,c)=>{const g=-d.depth*this.depthSpacing;i[c].z+=(g-r[c].z)*.35,i[c].y-=r[c].y*.08});const f=o/Math.max(this.iterations-1,1),m=this.idealEdgeLength*(.22*(1-f)+.01);r.forEach((d,c)=>{const g=i[c].length();g>1e-6&&d.addInPlace(i[c].scale(Math.min(g,m)/g))})}const u=r.reduce((o,i)=>o.addInPlace(i),l.Zero()).scaleInPlace(1/r.length);return r.map(o=>this.origin.add(o.subtract(u)))}}function Pn(e,n={}){const t=n.maxDepth??3,a=n.maxNodes??72,r=n.horizontalSpacing??.14,s=n.depthSpacing??.2,u=n.layoutIterations??120,o=n.origin?.clone()??new l(0,.1,0),i=[{node:e,parentIndex:null,depth:0}];for(let p=0;p<i.length&&i.length<a;p+=1){const h=i[p];if(!(h.depth>=t))for(const C of h.node.children){if(i.length>=a)break;i.push({node:C,parentIndex:p,depth:h.depth+1})}}const f=new Map;i.forEach((p,h)=>f.set(p.node.start,h));const m=i.flatMap((p,h)=>p.parentIndex===null?[]:[{sourceIndex:p.parentIndex,targetIndex:h,strength:.9}]),d=[],c=new Set,g=(p,h)=>{const C=f.get(p),w=f.get(h),R=`${p}:${h}`;C===void 0||w===void 0||C===w||c.has(R)||(c.add(R),d.push({sourceIndex:C,targetIndex:w,strength:.4}))};i.forEach(p=>{p.node.uses.forEach(h=>{const C=i[f.get(h)??-1]?.node,w=p.node.kind==="FunctionDeclaration"&&C?.kind==="FunctionDeclaration";g(w?p.node.start:h,w?h:p.node.start)}),p.node.usedBy.forEach(h=>{const C=i[f.get(h)??-1]?.node,w=p.node.kind==="FunctionDeclaration"&&C?.kind==="FunctionDeclaration";g(w?h:p.node.start,w?p.node.start:h)})});const v=new Array(i.length),P=new Bn(r,s,o,u).layout(i,[...m,...d]),M=i.map(()=>[]);i.forEach((p,h)=>{p.parentIndex!==null&&M[p.parentIndex].push(h)});const D=P.map(p=>p.clone()),z=P.map(()=>.04);for(let p=i.length-1;p>=0;p-=1){const h=M[p];if(h.length===0)continue;const C=h.reduce((ae,$)=>{const Z=z[$];return l.Minimize(ae,D[$].subtract(new l(Z,Z,Z)))},new l(Number.POSITIVE_INFINITY,Number.POSITIVE_INFINITY,Number.POSITIVE_INFINITY)),w=h.reduce((ae,$)=>{const Z=z[$];return l.Maximize(ae,D[$].add(new l(Z,Z,Z)))},new l(Number.NEGATIVE_INFINITY,Number.NEGATIVE_INFINITY,Number.NEGATIVE_INFINITY)),R=C.add(w).scale(.5),te=h.reduce((ae,$)=>Math.max(ae,l.Distance(R,D[$])+z[$]),0)+.08;D[p]=R,z[p]=te}return i.forEach((p,h)=>{const C=p.node.label,w=M[h].length>0,R=z[h]*2,te=_e(D[h],C,p.node.kind,w?new l(R,R,R):void 0,w?.1:1);te.name=`ast_${h}_${C.replace(/[^a-z0-9]+/gi,"_")}`,te.metadata={astKind:p.node.kind,astLabel:C,sourceStart:p.node.start,sourceEnd:p.node.end},v[h]=te}),i.forEach((p,h)=>{p.parentIndex!==null&&M[p.parentIndex].length>0&&se.set(v[h],v[p.parentIndex])}),d.forEach(p=>{if(M[p.sourceIndex].length>0&&M[p.targetIndex].length>0)return;const h=i[p.sourceIndex].node.start,C=i[p.targetIndex].node.start,w=je(v[p.sourceIndex],v[p.targetIndex],.3,"reference");w.metadata={...w.metadata,fromId:h,toId:C}}),v.length}function Re(e){const n=yn(e);return n?(fn(n),!0):!1}function Ae(e){const t=(e.grip?.absolutePosition?.clone()??e.pointer?.absolutePosition?.clone()??l.Zero()).add(new l(0,.02,0));_e(t),x(`📦 Placed box (${T.length})`)}function In(e){const n=ke(e)??E.get(e)??null;if(Ge(n),A&&I&&I!==e){Je(e)?Dn(e):x("🔗 Point the second controller at the destination node");return}if(I===e)return;const t=n;t?(A=t,I=e,x("🔗 Select the destination with the other controller")):(A=null,I=null,B?.dispose(),B=null,ne(),x("🔗 Point at a node to start a connection"))}function Dn(e){const n=I,t=Je(e);if(A&&t&&t!==A){if(je(A,t),n&&(Q.get(n)??!1)){B?.dispose(),B=null,x("🔗 Connection drawn — select another destination");return}x("🔗 Connection drawn")}else x("🔗 Connection cancelled");A=null,I=null,B?.dispose(),B=null,ne()}function Tn(){A=null,I=null,B?.dispose(),B=null,ne(),x("🔗 Connection cancelled")}function Fn(e){const n=e.motionController;if(!n)return;const t=He(e),a=t==="left",r=t==="right",s=n.getComponent("trigger")??n.getComponent("xr-standard-trigger")??n.getComponent("squeeze")??n.getComponentOfType("trigger")??n.getComponentOfType("squeeze");s&&s.onButtonStateChangedObservable.add(c=>{const g=Q.get(e)??!1;if(c.pressed&&!g){if(Fe(e)){x("📟 Read-only source tablet"),n.pulse?.(.15,50);return}Q.set(e,!0),In(e),n.pulse?.(.15,50)}else!c.pressed&&g&&(Q.set(e,!1),I===e&&Tn())});const u=n.getComponent("squeeze")??n.getComponentOfType("squeeze");u&&u.onButtonStateChangedObservable.add(c=>{if(c.pressed?de.add(e):de.delete(e),c.pressed){if(Fe(e))F=e,x("🤏 Tablet grabbed — release grip to place it");else{const g=ke(e,!0);if(!g){x("🎯 Hold grip on a box or tablet to move it"),n.pulse?.(.3,100);return}J.set(e,g);const v=e.grip?.absolutePosition?.clone()??e.pointer?.absolutePosition?.clone()??l.Zero(),P=(e.pointer?.getDirection?.(l.Forward())??l.Forward()).normalize(),M=g.position.subtract(v),D=l.Dot(M,P);le.set(e,D<.01?.15:D),x("🤏 Grip grabbed distant box")}n.pulse?.(.3,100)}else F===e?(F=null,x("✋ Tablet released")):J.has(e)&&(J.delete(e),le.delete(e),x("✋ Box released"))});const o=n.getComponent("a-button")??n.getComponent("x-button"),i=n.getComponent("x-button")??n.getComponent("y-button")??n.getComponent("a-button");o&&(r||!a&&!r)&&o.onButtonStateChangedObservable.add(c=>{c.pressed&&(Re(e)||me(),n.pulse?.(.2,70))}),i&&(a||!a&&!r)&&i.onButtonStateChangedObservable.add(c=>{c.pressed&&(Re(e)||me(),n.pulse?.(.2,70))});const f=n.getComponent("b-button")??n.getComponent("y-button")??n.getComponent("a-button");f&&(r||!a&&!r)&&f.onButtonStateChangedObservable.add(c=>{c.pressed&&(Ae(e),n.pulse?.(.2,70))});const m=n.getComponent("y-button");m&&a&&m.onButtonStateChangedObservable.add(c=>{c.pressed&&(Ae(e),n.pulse?.(.2,70))});const d=n.getComponent("menu")??n.getComponent("xr-standard-menu");if(d&&a){let c=!1;d.onButtonStateChangedObservable.add(g=>{g.pressed&&!c&&(me(),n.pulse?.(.2,70)),c=g.pressed})}x("🕶 Quest 3 controller layout ready")}async function Rn(){if(!(!L||O)&&L.baseExperience.state!==G.IN_XR){O=!0,x("▶️ Requesting AR session…");try{await L.baseExperience.enterXRAsync("immersive-ar","local-floor")}catch(e){console.error(e),x(`❌ AR request failed — ${e instanceof Error?e.message:"check Quest Browser permissions"}`)}finally{O=!1,fe()}}}async function An(){if(!(!L||O)&&L.baseExperience.state!==G.IN_XR){O=!0,x("▶️ Requesting VR session…");try{await L.baseExperience.enterXRAsync("immersive-vr","local-floor")}catch(e){console.error(e),x(`❌ VR request failed — ${e instanceof Error?e.message:"check Quest Browser permissions"}`)}finally{O=!1,fe()}}}async function Vn(){if(x("⏳ Initialising WebXR…"),typeof navigator.xr>"u"){x("❌ WebXR is not available in this browser");return}if(ue=await navigator.xr.isSessionSupported("immersive-ar").catch(()=>!1),ve=await navigator.xr.isSessionSupported("immersive-vr").catch(()=>!1),!ue&&!ve){x("❌ Immersive AR/VR is not supported on this device/browser");return}const e=await nn.CreateAsync(b,{disableDefaultUI:!0,disablePointerSelection:!0,disableTeleportation:!0,disableNearInteraction:!0,uiOptions:{sessionMode:ue?"immersive-ar":"immersive-vr"},optionalFeatures:!0});L=e,e.baseExperience.onStateChangedObservable.add(n=>{n===G.IN_XR?(be=!0,x("🟢 XR ready — Aim assist: optimized • Grip: grab/move • Trigger: draw connections • B/Y: place • X/A: delete")):n===G.NOT_IN_XR&&(be=!1,_(),H.clear(),x("⬜ XR not active")),fe()}),e.input.onControllerAddedObservable.add(n=>{K.push(n),n.onMotionControllerInitObservable.add(()=>{Fn(n)})}),e.input.onControllerRemovedObservable.add(n=>{const t=K.indexOf(n);t>=0&&K.splice(t,1);const a=ge.get(n);a&&(a.dispose(),ge.delete(n)),Q.delete(n),J.delete(n),le.delete(n),de.delete(n),F===n&&(F=null),_(n),H.delete(n)}),rn(),fe()}re&&re.addEventListener("click",async()=>{ye(),await Rn()});ie&&ie.addEventListener("click",async()=>{ye(),await An()});window.addEventListener("click",()=>{L?.baseExperience.state!==G.IN_XR&&ye()});for(const e of Array.from(document.querySelectorAll("[data-action]")))e.addEventListener("click",()=>{const n=e.getAttribute("data-action");if(n){if(n.startsWith("node-")){const t=n.replace("node-","").replace(/^./,a=>a.toUpperCase());x(`🧠 Selected ${t} node`),e.classList.add("active");for(const a of Array.from(document.querySelectorAll("[data-action]")))a!==e&&a.classList.remove("active")}sn(!1)}});b.registerBeforeRender(()=>{H.clear(),be&&L?.baseExperience.state===G.IN_XR&&(be=!1,me()),gn(b.getEngine().getDeltaTime()/1e3);for(const[e,n]of Array.from(J.entries())){const t=e.grip?.absolutePosition?.clone()??e.pointer?.absolutePosition?.clone()??l.Zero(),a=bn(e),r=hn(e),s=le.get(e)??.15,u=Math.abs(r)>1e-6?Math.max(.05,s+r):s;le.set(e,u);const o=t.add(a.depthAxis.scale(u));pn(n,o)}xn(),Ze();for(const e of K){const n=Y(e);n?.mesh||_(e),vn(e,n??void 0),Mn(e,n??void 0),I===e&&kn(e,n??void 0)}for(const e of X.keys())Oe(e)});we.runRenderLoop(()=>{b.render()});window.addEventListener("resize",()=>we.resize());Vn().catch(e=>{console.error(e),x("❌ WebXR could not start")});ye();
//# sourceMappingURL=astXr-DzSt19d6.js.map
