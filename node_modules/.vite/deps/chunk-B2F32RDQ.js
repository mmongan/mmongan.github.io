import {
  MaterialDefines
} from "./chunk-XDF2MLCS.js";
import {
  WebXRAbstractFeature,
  WebXRFeatureName,
  WebXRFeaturesManager
} from "./chunk-KCICUKZ4.js";
import {
  HemisphericLight
} from "./chunk-PMIKEK7U.js";
import {
  AbstractMesh,
  Axis,
  Camera,
...
import {
  ...
  UniqueIdGenerator,
  VertexBuffer
} from "./chunk-JBBQKTXI.js";
import {
  PointerEventTypes,
  PointerInfo
} from "./chunk-ZTNSNSOO.js";
import {
  Engine
} from "./chunk-JERSGHNX.js";
import {
  Node
} from "./chunk-BXRIRJUZ.js";
import {
  SerializationHelper,
  __decorate,
  serialize,
  serializeAsColor4
} from "./chunk-NNRABCOO.js";
import {
  ErrorCodes,
  IsBase64DataUrl,
  RandomGUID,
...
import {
  ...
  ShaderStore,
  ThinEngine
} from "./chunk-MVCZ7XDW.js";
import {
  EngineStore,
  Observable
} from "./chunk-3IBEUPNB.js";
import {
  Color3,
  Color4,
  Scalar,
  TmpColors
} from "./chunk-ZGSE2EJF.js";
import {
  ArrayTools,
  Epsilon,
  GetClass,
...
var Animation = class _Animation {
   ...
   */
  static _PrepareAnimation(name4, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction) {
    let dataType = void 0;
    ...
    const animation = new _Animation(name4, targetProperty, framePerSecond, dataType, loopMode);
    const keys = [
      { frame: 0, value: from },
   ...
   */
  static CreateAndStartAnimation(name4, target, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction, onAnimationEnd, scene) {
    const animation = _Animation._PrepareAnimation(name4, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction);
    if (!animation) {
      return null;
    }
    if (target.getScene) {
   ...
   */
  static CreateAndStartHierarchyAnimation(name4, node, directDescendantsOnly, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction, onAnimationEnd) {
    const animation = _Animation._PrepareAnimation(name4, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction);
    if (!animation) {
      return null;
    }
   ...
   */
  static CreateMergeAndStartAnimation(name4, node, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction, onAnimationEnd) {
    const animation = _Animation._PrepareAnimation(name4, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction);
    if (!animation) {
      return null;
    }
   ...
   */
  constructor(name4, targetProperty, framePerSecond, dataType, loopMode, enableBlending) {
    this.name = name4;
  ...
  toString(fullDetails) {
    ...
    if (fullDetails) {
      ret += ", Ranges: {";
      let first = true;
      for (const name4 in this._ranges) {
        if (first) {
          ret += ", ";
          first = false;
        }
   ...
   */
  createRange(name4, from, to) {
    if (!this._ranges[name4]) {
      this._ranges[name4] = new AnimationRange(name4, from, to);
    }
  }
  /**
   ...
   */
  deleteRange(name4, deleteFrames = true) {
    const range = this._ranges[name4];
    if (!range) {
      return;
    }
    if (deleteFrames) {
  ...
  clone() {
    ...
    if (this._ranges) {
      ...
      for (const name4 in this._ranges) {
        ...
        clone._ranges[name4] = range.clone();
      }
    }
    return clone;
  }
  ...
  serialize() {
    ...
    for (const name4 in this._ranges) {
      const source = this._ranges[name4];
      if (!source) {
        continue;
      }
      const range = {};
   ...
   */
  static ParseFromFileAsync(name4, url) {
...
Node._AnimationRangeFactory = (name4, from, to) => new AnimationRange(name4, from, to);
...
var SceneLoader = class _SceneLoader {
  ...
  static _LoadData(fileInfo, scene, onSuccess, onProgress, onError, onDispose, pluginExtension, name4) {
    ...
    const manifestChecked = () => {
      ...
      request = plugin.loadFile ? plugin.loadFile(scene, fileInfo.rawData || fileInfo.file || fileInfo.url, fileInfo.rootUrl, dataCallback, onProgress, useArrayBuffer, errorCallback, name4) : scene._loadFile(fileInfo.file || fileInfo.url, dataCallback, onProgress, true, useArrayBuffer, errorCallback);
    };
   ...
   */
  static ImportMesh(meshNames, rootUrl, sceneFilename = "", scene = EngineStore.LastCreatedScene, onSuccess = null, onProgress = null, onError = null, pluginExtension = null, name4 = "") {
    if (!scene) {
    ...
    }, progressHandler, errorHandler, disposeHandler, pluginExtension, name4);
  }
  /**
   * Import meshes into a scene
   * @param meshNames an array of mesh names, a single mesh name, or empty string for all meshes that filter what meshes are imported
   ...
   */
  static ImportMeshAsync(meshNames, rootUrl, sceneFilename = "", scene = EngineStore.LastCreatedScene, onProgress = null, pluginExtension = null, name4 = "") {
    return new Promise((resolve, reject) => {
   ...
   * @returns The loaded plugin
   */
  static Load(rootUrl, sceneFilename = "", engine = EngineStore.LastCreatedEngine, onSuccess = null, onProgress = null, onError = null, pluginExtension = null, name4 = "") {
    if (!engine) {
      Tools.Error("No engine available");
      return null;
    }
    return _SceneLoader.Append(rootUrl, sceneFilename, new Scene(engine), onSuccess, onProgress, onError, pluginExtension, name4);
  }
  /**
   * Load a scene
   * @param rootUrl a string that defines the root url for the scene and resources or the concatenation of rootURL and filename (e.g. http://example.com/test.glb)
   ...
   */
  static LoadAsync(rootUrl, sceneFilename = "", engine = EngineStore.LastCreatedEngine, onProgress = null, pluginExtension = null, name4 = "") {
    return new Promise((resolve, reject) => {
   ...
   * @returns The loaded plugin
   */
  static Append(rootUrl, sceneFilename = "", scene = EngineStore.LastCreatedScene, onSuccess = null, onProgress = null, onError = null, pluginExtension = null, name4 = "") {
    if (!scene) {
   ...
   */
  static AppendAsync(rootUrl, sceneFilename = "", scene = EngineStore.LastCreatedScene, onProgress = null, pluginExtension = null, name4 = "") {
    return new Promise((resolve, reject) => {
   ...
   * @returns The loaded plugin
   */
  static LoadAssetContainer(rootUrl, sceneFilename = "", scene = EngineStore.LastCreatedScene, onSuccess = null, onProgress = null, onError = null, pluginExtension = null, name4 = "") {
    if (!scene) {
...
var PostProcess = class _PostProcess {
  ...
  /** @internal */
  constructor(name4, fragmentUrl, parameters, samplers, _size, camera, samplingMode = 1, engine, reusable, defines = null, textureType = 0, vertexUrl = "postprocess", indexParameters, blockCompilation = false, textureFormat = 5, shaderLanguage = ShaderLanguage.GLSL) {
...
Texture._CreateRenderTargetTexture = (name4, renderTargetSize, scene, generateMipMaps, creationFlags) => {
  return new RenderTargetTexture(name4, renderTargetSize, scene, generateMipMaps);
};
...
//# sourceMappingURL=chunk-B2F32RDQ.js.map
